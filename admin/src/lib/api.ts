...
export async function updateNews(id: number | string, etag: string, payload: Partial<NewsItem>) {
  const res = await fetch(`${BASE}/${encodeURIComponent(String(id))}`, {
    ...
    headers: headers(etag), body: JSON.stringify(payload),
  });
  if (res.status === 422) throw await res.json();
...
/** Dashboard stats */
export async function getStats(): Promise<{ total: number; published: number; draft: number; archived: number }> {
